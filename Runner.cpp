...
int main(int argc, const char** argv) {
	...
	*/
	std::vector<std::string> files = selectCFiles(compilationDatabasePtr->getAllFiles());

	std::ofstream stream("run.sh");
	stream << "#!/bin/bash\n";
	for (const std::string& file: files) {
		stream << "build/Rewriter3 '" << argv[1] << "' '" << file << "'\n";
	}
